...
export default function ValuesSelectPageWithRealtime({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const [currentParticipant, setCurrentParticipant] = useState<{ id: number; name: string } | null>(null);
  ...
  useEffect(() => {
    async function fetchCurrentParticipant() {
      ...
      // Find participant by user's family member
  ...
  useEffect(() => {
    async function loadMySelections() {
      ...
      if (data) {
        setSelectedValues(new Set(data.map(s => s.value_name)));
      }
    }

    loadMySelections();
  }, [workshopId, currentParticipant]);
  ...
  const toggleValue = async (valueName: string) => {
    if (!currentParticipant) return;
  ...
  const handleSendMessage = async () => {
    ...
    await realtime.sendMessage(newMessage.trim());
    setNewMessage("");
  };
  ...
  // Get value popularity from real-time stats
  const getValuePopularity = (valueName: string): number => {
    const stat = realtime.valueStats[valueName];
    return stat?.percentage || 0;
  };

  // Calculate participant progress
  const getParticipantProgress = () => {
    const online = realtime.onlineParticipants.length;
    const total = realtime.participants.length;
    return { online, total };
  };
  ...
  return (
    <div className="min-h-screen bg-page-background flex flex-col">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Badge variant="outline" className="gap-2">
              ...
              {progress.online}/{progress.total} online
            </Badge>
            <Badge variant="outline" className="gap-2">
              <Clock className="h-3 w-3" />
              {formatTime(timeRemaining)}
            </Badge>
            <Badge variant={realtime.isConnected ? "default" : "secondary"} className="gap-2">
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 p-6 overflow-hidden">
        {/* Left Sidebar - Facilitator Panel */}
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                ...
                <p>• Participants ready: {realtime.onlineParticipants.length}/{realtime.participants.length}</p>
                <p>• Values selected: {Object.keys(realtime.valueStats).length}</p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="flex-1 overflow-auto p-6">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                {FAMILY_VALUES.map((value) => {
                  ...
                  const selectionCount = realtime.valueStats[value.name]?.count || 0;
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {selectionCount > 0 && (
                              <span className="text-xs text-muted-foreground">
                                {selectionCount} {selectionCount === 1 ? 'person' : 'people'}
                              </span>
                            )}
                          </div>
                        </div>
        ...
        <div className="col-span-3 flex flex-col gap-4 overflow-hidden">
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {realtime.onlineParticipants.map(p => (
                  <Avatar key={p.participant_id} className="h-8 w-8" title={p.participant_name}>
                  ...
                  {realtime.messages.map((msg) => (
                    <div key={msg.id} className="text-sm">
                      <div className="flex items-start gap-2">
                        {msg.message_type === 'ai' && <Bot className="h-4 w-4 text-blue-600 mt-0.5" />}
              ...
              <div className="p-3">
                <div className="flex gap-2">
                  <Input
                    ...
                    className="text-sm"
                  />
                  <Button size="sm" onClick={handleSendMessage} disabled={!currentParticipant}>
